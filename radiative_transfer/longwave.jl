...
function longwave_radiation(lw_sky, LAI, t_leaf, t_soil, Kb, Kd, em_leaf, em_soil)
  ## commonly used terms--
  ...
  ## Equation 14.134
  lw_down_trans(x) = lw_sky * (1 - em_leaf * (1-exp(-Kd * x)))
  lw_down_emit(x) = lw_leaf_emit *(1-exp(-Kd * x))

  lw_down(LAI) = lw_down_emit(LAI) + lw_down_trans(LAI)


  ## Equation 14.135

  lw_up_trans(x) = lw_soil_emit * (1 - em_leaf * (1-exp(-Kd * (LAI - x))))


  lw_up_emit(x) = lw_leaf_emit * (1-exp(-Kd * (LAI - x)))

  lw_up(x) = lw_up_trans(x) + lw_up_emit(x)
  ## Equation 14.137
  ...
  return [lc, lg, lc_sun, lc_sha,  lw_up(0), lw_down(LAI)]
end