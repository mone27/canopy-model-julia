...
function direct_beam_radiation(sw_sky_b, LAI, Kb, Kd, beta, beta0, omega_leaf, clump_OMEGA, alb_soil_b, alb_soil_d)
    ...
    s1(x) = exp(-h * clump_OMEGA * x);
    s2(x) = exp(- Kb * clump_OMEGA * x)
    ...
    i_upw_b(x) = -g1 * s2(x) + n1b * u * s1(x) + n2b * v / s1(x)
    i_dwn_b(x) =  g2 * s2(x) - n1b * v * s1(x) - n2b * u / s1(x)
...
function diffuse_radiation(sw_sky_d, LAI, Kb, Kd, beta, beta0, omega_leaf, clump_OMEGA, alb_soil_d)
  ...
  i_upw_d(x) =  n1d * u * s1(x) + n2d * v / s1(x)
  i_dwn_d(x) = -n1d * v * s1(x) - n2d * u / s1(x)
...
#'
#' @return list of ic, ic_sun, ic_sha
function shortwave_radiation(sw_sky_b, sw_sky_d, LAI, Kb, Kd, beta, beta0, omega_leaf, clump_OMEGA, alb_soil_b, alb_soil_d)
